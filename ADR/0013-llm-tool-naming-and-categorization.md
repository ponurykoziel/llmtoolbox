...
The naming convention admits rare, explicitly approved exceptions. An exception must be recorded in this section before the tool ships; anything not listed here follows the standard convention.

- **`sleep`** — allowed as a category-less operationId. `sleep` is a flow-control primitive, not an operation on any semantic domain: there is no category to narrow from, and the bare verb is unambiguous to every LLM. Approved 2026-09-06.
