...
@Path("/api/tools/fs/tail")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class TailResource extends FsResourceSupport implements ToolBean {
    ...
    @Operation(
            operationId = "fs_tail",
            summary = "Returns the last N lines (default 200, max 5000) of a regular file inside the allowed root"
    )
    @POST
    @Path("/execute")
    public ExecutionResponse execute(TailRequestDto request) {
        ...
        String command = "tail -n " + lines + " -- " + ToolSupport.shellQuote(path);
